...
/** LIST (public) — optional ?course=<id> (no auth filter to avoid breaking current flows) */
r.get('/', async (req, res) => {
  const filter = {};
...
/** BULK create (auth) — each item MUST include course that belongs to the caller */
r.post('/bulk', authBearer, async (req, res) => {
  ...
  // Collect distinct course ids to verify they all belong to this user
  const ids = [...new Set(arr.map(i => i?.course).filter(Boolean))];
  if (ids.length === 0) return res.status(400).json({ success:false, message:'Each item must include course' });
  if (!ids.every(id => mongoose.isValidObjectId(id))) return res.status(400).json({ success:false, message:'Invalid course id in payload' });
  ...
  for (const [idx, i] of arr.entries()) {
    ...
    if (!Array.isArray(i?.questions)) continue;
...
/** CREATE (auth) — course must exist and belong to user */
...
/** UPDATE (auth + owner) — if changing course, new course must also be owned */
